...
exports.validateUpload = (req, res, next) => {
    ...
    if (validDescriptions.indexOf(description) > -1 ) {
        if (description === "profileImage") {
            if (!userId) res.status(400).json({"error": "true", "message": "Missing 'userID'"})
            else next()
        } else if (description === "postImage") {
            if (!postId) res.status(400).json({"error": "true", "message": "Missing 'postID'"})
            else next()
        } else if (description === "commentImage") {
            if (!commentId) res.status(400).json({"error": "true", "message": "Missing 'commentID'"})
            else next()
        }
    } else {
