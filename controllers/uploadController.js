  ...
  try {
    ...
    if (fileType === '.png' || fileType === '.jpg') {
      ...
      if (description === 'profileImage') {
        ...
        await Post.findOneAndUpdate({_id: postId}, {image: newImage._id});
      } else if (description === 'commentImage') {
        await Comment.findOneAndUpdate({_id: commentId}, {image: newImage._id});
      }
      ...
      fs.unlink(tempPath, (err) => {
        ...
        res
          .status(200)
          .json({error: true, message: 'Only .png files are allowed!'});
      });
    }
  } catch (err) {
